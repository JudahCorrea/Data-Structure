...
test("Para qualquer conjunto A, ∅ ⊆ A", () => {
	expect(A.contains(new Sets())).toBe(true);
});

//novos
test("A ∩ ∅ = ∅", () => {
	let c3 = A.intersection(new Sets());
	expect(c3.isEqual(new Sets())).toBe(true);
});

test("A U ∅ = A", () => {
	let c3 = A.union(new Sets());
